...
const SelectedEmoji = ({ symbol, label, i }: SelectedEmojiProps) => {
    ...
    return (
        <CSSTransition key={symbol} classNames="fade" timeout={250}>
            <div
                onMouseEnter={() => setIsHovering(true)}
                onMouseLeave={() => setIsHovering(false)}
                className={`relative text-8xl flex justify-center items-center bg-white shadow-md rounded-3xl  ${
            ...
            >
                <Emoji symbol={symbol} label={label} classString="mb-3 z-10" />
                <CSSTransition
                    in={isHovering}
                    key={`${symbol}-selected`}
                    classNames="scale"
